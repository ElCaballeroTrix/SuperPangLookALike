...
void CORE_RenderSprite(const vec2 &p0, const vec2 &p1, GLuint texid, double xCoordinates, double yCoordinates, double sizePercentageX, double sizePercentageY)
{
  GLdouble maxCoordX = xCoordinates + static_cast<GLdouble>(sizePercentageX);
  GLdouble maxCoordY = yCoordinates + static_cast<GLdouble>(sizePercentageY);
  glBindTexture( GL_TEXTURE_2D, texid );
  glBegin( GL_QUADS );
  glTexCoord2d(xCoordinates,yCoordinates); glVertex2f(p0.x, p0.y);
  glTexCoord2d(maxCoordX,yCoordinates); glVertex2f(p1.x, p0.y);
  glTexCoord2d(maxCoordX, maxCoordY); glVertex2f(p1.x, p1.y);
  glTexCoord2d(xCoordinates, maxCoordY); glVertex2f(p0.x, p1.y);
  glEnd();
}
